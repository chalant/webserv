...
MockLoggerConfiguration::MockLoggerConfiguration(
    IBufferManager &bufferManager, const IConfiguration &configuration,
    IPollfdManager &pollfdManager)
    : _errorLogFile(""), _accessLogFile(""), _bufferManager(bufferManager),
      _bufferSize(99999), _errorLogFileDescriptor(STDOUT_FILENO),
      _accessLogFileDescriptor(STDOUT_FILENO), _errorLogEnabled(true),
...
void MockLoggerConfiguration::setFileDescriptor(int errorLogFileDescriptor,
                                                int accessLogFileDescriptor)
{
