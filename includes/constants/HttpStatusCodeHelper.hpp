...
enum HttpStatusCode
{
    ...
    NON_AUTHORITATIVE_INFORMATION = 203, // The server is a transforming proxy that received a 200 OK response from its origin, but is returning a modified version of the origin's response
    NO_CONTENT = 204,                    // The server successfully processed the request and is not returning any content
    RESET_CONTENT = 205,                 // The server successfully processed the request and is not returning any content, but the client should reset the document view
    ...
    NOT_MODIFIED = 304,       // Indicates that the resource has not been modified since the version specified by the request headers If-Modified-Since or If-None-Match
    ...
    PERMANENT_REDIRECT = 308, // This and all future requests should be directed to the given URI
    ...
    REQUEST_HEADER_FIELDS_TOO_LARGE = 431, // Server is unwilling to process the request because either an individual header field or all the header fields collectively are too large
    UNAVAILABLE_FOR_LEGAL_REASONS = 451,   // Resource access is denied for legal reasons
    ...
    SERVICE_UNAVAILABLE = 503,            // The server cannot handle the request due to temporary overload or maintenance of the server
...
private:
    // Member variables
    const std::vector<std::string> _statusCodeList;                       // List of string representations of HTTP versions
    const std::map<std::string, HttpStatusCode> _stringHttpStatusCodeMap; // Map of string representations to HttpStatusCode enum values
    const std::map<HttpStatusCode, std::string> _httpStatusCodeStringMap; // Map of HttpStatusCode enum values to string representations
