...
enum HttpHeader
{
    ...
    TRANSFER_ENCODING, // Specifies the form of encoding used to safely transfer the entity to the user
    UPGRADE,           // Requests that the server upgrade to a different protocol
    VIA,               // Informs the recipient about the network path taken by the request message
    WARNING,           // Contains additional information about potential problems encountered by the server

    // Request Headers
    ACCEPT,              // Indicates the media types that are acceptable for the response
    ACCEPT_CHARSET,      // Indicates the character sets that are acceptable for the response
    ACCEPT_ENCODING,     // Indicates the encoding algorithms that are acceptable for the response
    ACCEPT_LANGUAGE,     // Indicates the natural languages that are preferred for the response
    ...
    IF_MATCH,            // Makes the request conditional on the current representation of the target resource
    IF_MODIFIED_SINCE,   // Makes the request conditional on the last modification date of the target resource
    IF_NONE_MATCH,       // Makes the request conditional on a current representation of the target resource not matching any of the given ETags
    IF_RANGE,            // Makes the request conditional on a range of the target resource
    IF_UNMODIFIED_SINCE, // Makes the request conditional on the last modification date of the target resource not being later than the given date/time
    MAX_FORWARDS,        // Indicates the remaining number of times that this request message can be forwarded
    PROXY_AUTHORIZATION, // Contains the credentials to authenticate the user agent with a proxy server
    ...
    USER_AGENT,          // Contains information about the user agent originating the request
    ...
    PROXY_AUTHENTICATE, // Contains the credentials to authenticate the user agent with a proxy server
    ...
    VARY,               // Specifies the request header fields that influence the response representation's selection
    WWW_AUTHENTICATE,   // Contains the credentials to authenticate the user agent with the origin server

    // Entity Headers
    ALLOW,            // Specifies the set of HTTP methods that are supported for the target resource
    CONTENT_ENCODING, // Specifies the encoding that has been applied to the representation
    CONTENT_LANGUAGE, // Specifies the natural language(s) of the intended audience for the representation
    ...
    CONTENT_RANGE,    // Indicates where in a full body message this partial message belongs
    ...
    // Additional Headers
    ACCESS_CONTROL_ALLOW_ORIGIN,         // Specifies which origins are allowed to access the resource
    ACCESS_CONTROL_ALLOW_METHODS,        // Specifies the methods allowed when accessing the resource
    ACCESS_CONTROL_ALLOW_HEADERS,        // Specifies the headers allowed when accessing the resource
    ...
    ACCESS_CONTROL_REQUEST_METHOD,       // Used in response to a preflight request to indicate which HTTP methods can be used during the actual request
    ACCESS_CONTROL_REQUEST_HEADERS,      // Used in response to a preflight request to indicate which HTTP headers can be used during the actual request
    CONTENT_DISPOSITION,                 // Indicates the disposition type and optional filename of the content
    CONTENT_SECURITY_POLICY,             // Defines a policy to mitigate cross-site scripting (XSS) attacks
    CONTENT_SECURITY_POLICY_REPORT_ONLY, // Specifies a policy to mitigate cross-site scripting (XSS) attacks, but only reports violations
    DNT,                                 // Indicates whether the user agent prefers not to have the user's activity tracked
    ...
    TIMING_ALLOW_ORIGIN,                 // Indicates whether the resource is accessible to the requesting origin
...
private:
    // Member variables
    const std::vector<std::string> _headerList;                   // List of string representations of HTTP headers
    const std::map<std::string, HttpHeader> _stringHttpHeaderMap; // Map of string representations to HttpHeader enum values
    const std::map<HttpHeader, std::string> _httpHeaderStringMap; // Map of HttpHeader enum values to string representations
