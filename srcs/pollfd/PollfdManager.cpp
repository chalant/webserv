...
void PollfdManager::_addPollfd(pollfd pollFd) { this->_pollfds.push(pollFd); }

// Method to add a regular file pollfd to the pollfdQueue
void PollfdManager::addRegularFilePollfd(pollfd pollFd)
{
    if (this->_descriptorTypeMap.find(pollFd.fd) !=
        this->_descriptorTypeMap.end())
...
short PollfdManager::getEvents(int position)
{
    ...
    return (this->_pollfds[ position ].revents & 0x3F) | type;
}
...
pollfd *PollfdManager::getPollfdArray() { return this->_pollfds.data(); }
