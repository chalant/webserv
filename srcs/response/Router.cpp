...
Router::~Router()
{
    for (std::vector<IRoute *>::iterator it = m_routes.begin();
         it != m_routes.end(); ++it)
    {
        delete *it;
    }
...
void Router::m_createRoutes(IConfiguration *server_block)
{
    ...
    for (hostname_it = hostnames.begin(); hostname_it != hostnames.end();
         hostname_it++)
    {
        for (std::vector<IConfiguration *>::const_iterator location_it =
                 locations.begin();
             location_it != locations.end(); ++location_it)
        {
            ...
            for (port_it = ports.begin(); port_it != ports.end(); port_it++)
            {
                ...
                m_routes[ i ]->setMethod(methods, this->m_http_helper);
                i++;
            }
...
Triplet_t Router::execRoute(IRequest *req, IResponse *res)
{
    ...
    for (route = m_routes.begin(); route != m_routes.end(); route++)
    {
        if (req->getHostName() == (*route)->getHostname() &&
            ...
            std::find((*route)->getMethod().begin(),
                      (*route)->getMethod().end(),
        ...
        {
            // Exact match
            return (*route)->getResponseGenerator()->generateResponse(
                **route, *req, *res, this->m_configuration);
        }
    }
    for (route = m_routes.begin(); route != m_routes.end(); route++)
    {
...
IResponseGenerator *Route::getResponseGenerator(void) const
{
...
void Route::setHostname(std::string newHostname)
{
    this->m_host_name = newHostname;
}
...
void Route::setMethod(const std::vector<std::string> newMethods,
                      HttpHelper &httpHelper)
{
    // Set the method of the request
    for (size_t i = 0; i < newMethods.size(); i++)
    {
        this->m_methods.push_back(
            httpHelper.stringHttpMethodMap(newMethods[ i ]));
    }
}
